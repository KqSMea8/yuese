...
//          infoUrl = new URL("http://ip168.com/");
	...
	public ArrayList<Member_01152> Json_gukeinfo(String json) {
		ArrayList<Member_01152> list = new ArrayList<Member_01152>();
		try {
			JSONArray jsonArray = new JSONArray(json);
			for(int i=0;i<jsonArray.length();i++){
				...
				bean.setStar_ranking(item.getString("star"));
				bean.setPhoto(item.getString("photo"));
				...
				bean.setdailishang(item.getInt("agent_level"));
				list.add(bean);
			}
		} catch (Exception e) {
		...
		ArrayList<Member_01152> list = new ArrayList<Member_01152>();
  		try {
  			...
  			for(int i=0;i<jsonArray.length();i++){
				...
				bean.setMyrenzheng(item.getInt("myrenzheng"));
				bean.setdailishang(item.getInt("agent_level"));
				bean.setQrcodeimg(item.getString("qrcode"));
				bean.setReward_percent(item.getString("reward_percent"));
  				list.add(bean);
  			}
  		} catch (Exception e) {
  			LogDetect.send(LogDetect.DataType.specialType, "HelpManager_01152主播资料修改:", "=============");
