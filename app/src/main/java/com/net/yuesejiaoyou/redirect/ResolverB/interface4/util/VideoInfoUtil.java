...
public class VideoInfoUtil {
	...
	private static int reqTime = 0; // 请求视频封面信息的次数
...
//    }
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					String coverUrl;
					reqTime = 0;
					while((coverUrl = videoInfoRequest(videoid)) == null && reqTime < 20) {
						try {
							Thread.sleep(1000);
					...
					if(coverUrl == null) {
						callback.onGetCoverFail("over 10 times");
					} else {
						callback.onGetCoverSuccess(coverUrl);
					}
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static String videoInfoRequest(final String videoid) throws IOException {

		// 生成URL
		Log.v("TT","getVideoInfo(): "+videoid);
		Map<String, String> privateParams = generatePrivateParamters(videoid);
		Map<String, String> publicParams = generatePublicParamters();
		final String URL = generateOpenAPIURL(publicParams, privateParams);
		Log.v("TCQO","URL:"+URL);
		...
		Log.v("TCQO","code: "+code);
		if(code == 200) {
			String json = response.body().string();
			JSONObject jsonObj = JSONObject.fromObject(json);
			Log.v("TCQO","json:"+json);

			if(jsonObj.has("Code")) {	// 没有请求到视频信息则过一秒后再请求
				return null;
			} else {
				JSONObject jsonBase = jsonObj.getJSONObject("VideoBase");
				String coverUrl = jsonBase.getString("CoverURL");
				return coverUrl;
			}
		} else {
			return null;
...
//        privateParams.put("FileName", "1510130201331.mp4");
	...
	private static String getCQS(List<String> allParams) {
		VideoInfoUtil.ParamsComparator paramsComparator = new VideoInfoUtil.ParamsComparator();
