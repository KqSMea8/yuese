...
import com.net.yuesejiaoyou.redirect.ResolverC.getset.User_data;
...
public class HelpManager_01066 {
   ...
   public static List<String> getCity(Context mContext, String province) {
	   ...
	   if(cityMapList.get(province) == null) {

	   } else {
		   for (int i = 0; i < cityMapList.get(province).size(); i++) {
			   copyList.add(cityMapList.get(province).get(i));
		   }
	   }
	...
	private static void jsonInit(Context mContext) {

		nationList = new ArrayList<String>();
		provinceMapList = new HashMap<String, List<String>>();
		cityMapList = new HashMap<String, List<String>>();
		net.sf.json.JSONObject jsonObject = net.sf.json.JSONObject.fromObject(getJson(mContext, "area.json"));

		for(Object key : jsonObject.keySet()) {
			// 国家块
			net.sf.json.JSONObject nationObj = (net.sf.json.JSONObject) jsonObject.get(key);
			// 获取国家名称
			String nationName = (String)nationObj.get("n");
			nationList.add(nationName);

			List<String> provinceList = new ArrayList<String>();
			for (Object subKey : nationObj.keySet()) {
				if (!subKey.equals("n")) {
					net.sf.json.JSONObject provinceObj = (net.sf.json.JSONObject) nationObj.get(subKey);
					// 获取省名称
					String provinceName = (String)provinceObj.get("n");
					if(provinceName != null && !provinceName.isEmpty()) {
						provinceList.add(provinceName);
					}
					List<String> cityList = new ArrayList<String>();
					for(Object subsubKey : provinceObj.keySet()) {
						if(!subsubKey.equals("n")) {
							net.sf.json.JSONObject cityObj = (net.sf.json.JSONObject)provinceObj.get(subsubKey);
							String cityName = (String)cityObj.get("n");
							cityList.add(cityName);
						}
					}
					//
					cityMapList.put(provinceName.isEmpty()?nationName:provinceName, cityList);
					//Log.v("TT","nationName: "+nationName);
//					for(int i = 0; i < cityList.size(); i++) {
//						Log.v("TT",cityList.get(i));
//					}
				}
			}
			provinceMapList.put(nationName, provinceList);
		}

	//	LogDetect.send("01107","nationList: "+nationList);
	//	LogDetect.send("01107","provinceMapList: "+provinceMapList);
	//	LogDetect.send("01107","cityMapList: "+cityMapList);
	}

	public static List<String> getNation(Context mContext) {
		...
		for(int i = 0; i < nationList.size(); i++) {
			copyList.add(nationList.get(i));
		}

		return copyList;
	}
	public static List<String> getsheng(Context mContext, String name) {
...
//        Map<String,List<String>> shengMap = new HashMap<String,List<String>>();
//
//        for(String key : provinceMapList.keySet()) {
//            shengMap.put(key,provinceMapList.get(key));
//        }

		// 返回copy的list
		List<String> copyList = new ArrayList<String>();
		if(provinceMapList.get(name) == null) {

		} else {
			for (int i = 0; i < provinceMapList.get(name).size(); i++) {
				copyList.add(provinceMapList.get(name).get(i));
			}
		}
		...
		//return shengMap;
		//return provinceMapList.get(name);
	}
    ...
    public Videoinfo video_info(String json) {
		LogDetect.send(LogDetect.DataType.specialType, "json返回",json);
        Videoinfo bean=new Videoinfo();
        try {
            JSONArray jsonArray = new JSONArray(json);
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject item = jsonArray.getJSONObject(i);
                    ...
                    bean.setIsguanzhu(item.getInt("isguanzhu"));
                bean.setPay_num(item.getInt("pay_num"));
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject item = jsonArray.getJSONObject(i);
                    ...
                    bean.setRecieve_percent(item.getString("jieting"));
            }
