...
//import com.example.vliao.core.YhApplication;
...
public class Download {  
    ...
    {  
        this.urlstr = url;  
        LogDetect.send(LogDetect.DataType.basicType, "apk下载链接: ", url);
        //获取设备sd卡目录  
    ...
    {  
        ...
        if (!file.exists()) {  
            file.mkdirs();  
            //创建文件夹  
//           if(YhApplication.DEBUG_FLAG)Log.d("log", sb.toString());
        }  
        //获取文件全名  
        sb.append("/"+filename);  
        ...
        try {  
        	...
        	logDbg.send(LogDetect.DataType.exceptionType, "下载APK发生异常 ", e.getStackTrace().toString());
            return 0;  
        } finally {  
            try {  
                fos.close();  
