...
public class Util {
    ...
    public static void sendMsgText1(final Context context, final String touser, final String nickname, final String headpic, final MyCallBack callBack) {

        SimpleDateFormat sd = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        getChatInfoTo(context, sd, Const.MSG_TYPE_DAZHAOHU, touser);

        String content = Util.nickname + "给你发来一条视频请求";
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    //LogDetect.send(DataType.noType,Utils.seller_id+"=phone="+Utils.android,"before sendMessage()");
                    if (sendMessage1(Utils.xmppConnection, message, touser)) {
                        callBack.success();
                    } else {
                        callBack.error();
                    }
    ...
    public static boolean sendMessage1(XMPPTCPConnection mXMPPConnection, String content,
                                       String touser) throws XMPPException, SmackException.NotConnectedException {
        if (mXMPPConnection == null || !mXMPPConnection.isConnected()) {
            // throw new XMPPException();
        }
