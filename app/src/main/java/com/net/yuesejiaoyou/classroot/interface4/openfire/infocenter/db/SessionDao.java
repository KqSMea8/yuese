...
public class SessionDao {
	...
	public long insertSession(Session session) {
		...
		values.put(DBcolumns.SESSION_HEADPIC, session.getHeadpic());
	...
	// 返回全部列表
		public List<Session> queryAllSessions(String user_id) {
			...
			while (cursor.moveToNext()) {
				...
				String headpic=cursor.getString(cursor.getColumnIndex(DBcolumns.SESSION_HEADPIC));
				...
				if(type.equals(Const.MSG_TYPE_URL)){
					String[] a=content.split("\\$");
					session.setContent(a[2]);
					list.add(session);
				}else if(type.equals(Const.MSG_TYPE_ORDER)){
					String[] a=content.split("\\$");
					session.setContent(a[8]);
					list.set(1, session);
				}else if(type.equals(Const.MSG_TYPE_SYSTEM)){
					String[] a=content.split("\\$");
					session.setContent(a[0]);
					list.set(0, session);
				}/*else if(type.equals(Const.MSG_TYPE_URL)){
					String[] a=content.split("\\$");
					session.setContent(a[2]);
					list.set(0, session);
				}*/
	...
	public void updateSessionToDisPose(/*String sessionid*/){
		...
		db.update(DBcolumns.TABLE_SESSION, values,/* DBcolumns.SESSION_id + " = ? ", new String[] { sessionid }*/null,null);
	}
